...
$sql = "select mb_level, mb_login_cnt, mb_nick, mb_updated_date from remember_member where mb_user='".trim($_SESSION['tw_id'])."'";
...
if ($num_record == 1){
	...
	$sql = "update remember_member set mb_date = now(), mb_ip = '".$_SERVER['REMOTE_ADDR']."', mb_agent = '".$_SERVER['HTTP_USER_AGENT']."', mb_login_cnt = ".$mb_login_cnt_plus.", mb_thumbnail = '".$_SESSION['tw_profile_image']."', mb_twitter_user = '".$_SESSION['tw_screen_user']."' where mb_user = '".trim($_SESSION['tw_id'])."'";
	mysql_query($sql, $connect);
	
	$sql = "select * from remember_boardset where mb_user = '".trim($_SESSION['tw_id'])."'";
	$result = mysql_query($sql, $connect);
	$row = mysql_fetch_array($result);
	...
	//포인트 합계
	$_SESSION['point'] = get_point_sum(trim($_SESSION['tw_id']));
	
	$_SESSION['user'] = trim($_SESSION['tw_id']);
	...
	$sql = "insert into remember_member (mb_id, mb_user, mb_updated_date, mb_ip, mb_agent, mb_facebook, mb_nick, mb_profile, mb_thumbnail, mb_twitter_user) ";
	$sql .= "values (".$max_mb_id.", '".trim($_SESSION['tw_id'])."', now(), '".$_SERVER['REMOTE_ADDR']."', '".$_SERVER['HTTP_USER_AGENT']."', 2, '".trim($_SESSION['tw_name'])."', '".trim($_SESSION['tw_profile'])."', '".trim($_SESSION['tw_profile_image'])."', '".trim($_SESSION['tw_screen_user'])."')";
	mysql_query($sql, $connect);
	
	$sql = "CREATE TABLE `remember_board_".trim($_SESSION['tw_id'])."` (";
	...
	$sql = "insert into remember_board_".trim($_SESSION['tw_id'])." (bo_id, bo_public, su_short_url, bo_content, bo_date, bo_ip, bo_option) ";
	$sql .= "values (1, 0, '".$short_url_random."', '<strong>".trim($_SESSION['tw_name'])."님, 환영합니다.</strong><br /><br />";
	...
	$sql .= "values (".$fr_id.", 'admin', '".trim($_SESSION['tw_id'])."', now())";
	mysql_query($sql, $connect);
	
	//open API
	$openapikey = substr(md5(date("YmdHis")), 0, 20).get_rand(5, "lower");
	$openapisecret = substr(md5(trim($tw_user)), 0, 20).get_rand(5, "lower");
	...
	//내주소
	$tmp_user_url = substr(str_replace(".", "_", $_SESSION['tw_screen_id']), 0, 20);
	...
	if ($user_cnt == 1){
		...
		$sql .= "values (".$max_bs_id.", '".trim($_SESSION['tw_id'])."', '".trim($_SESSION['tw_name'])."\'z', '".substr($tmp_user_url, 0, 15).get_rand(5, 'num')."', '".$openapikey."', '".$openapisecret."')";
	}else{
		$sql = "insert into remember_boardset (bs_id, mb_user, bs_subject, bs_user_url, bs_openapikey, bs_openapisecret) ";
		$sql .= "values (".$max_bs_id.", '".trim($_SESSION['tw_id'])."', '".trim($_SESSION['tw_name'])."\'z', '".$tmp_user_url."', '".$openapikey."', '".$openapisecret."')";
	}
	$result = mysql_query($sql, $connect);
	...
	$_SESSION['user'] = trim($_SESSION['tw_id']);
	$_SESSION['level'] = 0;
	$_SESSION['nick'] = trim($_SESSION['tw_name']);
