...
//$sql = "select * from remember_board_".trim($_GET['user'])." where bo_id = ".$_GET['id']." and bo_blacklist_date = '0000-00-00 00:00:00'";
$sql = "select * from remember_board_".trim($_GET['user'])." where bo_recycle_bin != 9 and bo_id = ".$_GET['id'];
$result = mysql_query($sql, $connect);
$row = mysql_fetch_array($result);

if ($_COOKIE["view_{$_GET[id]}"] != $_GET['id']){
	$sql = "update remember_board_".trim($_GET['user'])." set bo_hit = bo_hit + 1 where bo_id = ".$_GET['id'];
...
$short_url = $common_path.$su_short_url;
$bo_good = $row['bo_good'];
...
<div id="date_ip">
    <div id="nav">
		<p><? echo $bo_date ?>&nbsp;<? echo $bo_ip ?>&nbsp;<span title="조회수"><? echo $bo_hit ?></span>/<span title="스크랩"><? echo get_scrap_num($su_short_url) ?></span>&nbsp;<img src="/skin/<? echo $public_skin ?>/img/lock<? echo ($bo_public == 1) ? "" : "ed" ?>.png" /></p>
	</div>    
</div>
...
<div id="cont">
    ...
    <? }else{ ?>
    	...
    	<div id="contentstr" style="word-wrap:break-word;"><? echo $bo_content_str ?></div>
    <? } ?>
    	<p id="shorturlstr">post by <span style="font-size:11px; font-style:normal"><? echo get_profile($_GET['user']) ?></span> / <? if ($bo_public == 1) { ?><a href="<? echo $short_url ?>" id="shorturl"><? echo $short_url ?></a> <a href="#" id="copy-shorturl">(copy)</a><? } ?></p>   
    ...
    <div id="url_qrcode">
    	<div id="url">
            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
              	<!--<? //if ($mb_facebook == 1){ ?><td width="51"><a href="http://www.facebook.com/profile.php?id=<? echo $_GET['user'] ?>&sk=info" target="_blank"><img src="https://graph.facebook.com/<? echo $_GET['user'] ?>/picture"></td><? //} ?>-->
                <td background="/skin/<? echo $public_skin ?>/img/icon_copy.png" width="48" height="48" id="copy_table"><a href="#set" id="copy-content">&nbsp;&nbsp;copy</a></td>
                <td><? if ($bo_public == 1){ ?><? if (stristr($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== FALSE) { ?><a href="#set" onClick="window.external.AddFavorite('<? echo $short_url ?>', '<? echo $common_title." - ".get_mb_strimwidth($bo_content_title, 30) ?>')"><img src="/skin/<? echo $public_skin ?>/img/icon_add.png" title="즐겨찾기" /></a><? }} ?>
            <a href="#set" onclick="MM_openBrWindow('/print.short.php?user=<? echo $_GET['user'] ?>&id=<? echo $_GET['id'] ?>','print','scrollbars=yes,width=550,height=650')" ><img src="/skin/<? echo $public_skin ?>/img/icon_print.png" title="인쇄" /></a>
            
            	<? if ($_SESSION['user'] == $_GET['user']) { ?>
                    <? if ($bo_public == 1){ ?><a href="#" id="closed"><img src="/skin/<? echo $public_skin ?>/img/icon_lock.png" title="공개" /></a>
                    <? }else if ($bo_public == 0){ ?><a href="#" id="public"><img src="/skin/<? echo $public_skin ?>/img/icon_locked.png" title="비공개" /></a><? } ?>
                    <? if ($bo_public == 1){ ?><a href="#set" id="option"><img src="/skin/<? echo $public_skin ?>/img/icon_set.png" title="옵션" /></a><? } ?>
                    <? if ($_SESSION['set_recycle_bin'] == 1){ ?><a href="#set" id="del"><img src="/skin/<? echo $common_skin ?>/img/icon_del.png" title="삭제" /></a><? }else if ($_SESSION['set_recycle_bin'] == 0){ ?><a href="#set" id="recycle_bin"><img src="/skin/<? echo $common_skin ?>/img/icon_recycle_bin.png" title="휴지통" /></a><? } ?>
                <? }else if (isset($_SESSION['user'])){ ?>
                	<a href="#" id="scrap_input" title="스크랩"><img src="/skin/<? echo $public_skin ?>/img/icon_scrap.png" title="스크랩" /></a>
                    <a href="#" id="spam" title="신고"><img src="/skin/<? echo $public_skin ?>/img/icon_spam.png" title="신고" /></a>
         ...
         <? if (($bo_public == 1) and ($use_qrcode == 0)) { ?>
            <div id="qr_code">
                <img src="http://chart.apis.google.com/chart?cht=qr&chs=80x80&choe=UTF-8&chld=H&chl=<? echo urlencode($short_url) ?>&chld=L|0" title='<? echo get_mb_strimwidth($bo_content_title, 30) ?>'>
            </div>    
         <? } ?>
...
<? if ($bo_public == 1){ ?>
	<? if ($use_sns == 0){ ?>
        ...
        <a href="javascript:pstMe2Day('<? echo get_mb_strimwidth($bo_content_title, 40) ?>', '<? echo $short_url ?>')"><img src="/skin/<? echo $public_skin ?>/img/icon_me2day.png" /></a>
        <a href="javascript:pstYozmDaum('<? echo $short_url ?>','<? echo get_mb_strimwidth($bo_content_title, 40) ?>','<? echo $common_path ?>');return false;"><img src="/skin/<? echo $public_skin ?>/img/icon_yozm.png" /></a>
        </div>
...
<? 
//include_once ("skin/{$public_skin}/public/prev.next.php");
include_once ("prev.next.php");
...
$(document).ready(function(){
	...
	$( "a#spam" ).click(function() {
		if ($.cookie('cookie_spam') != 'spam_<? echo $su_short_url ?>'){
			$( "#hidden-message-spam" ).dialog({
				resizable: false,
				width: 500,
				modal: true,
				buttons: {
					"확인": function() {
						$.post('/process/spam.php', $("form#spam").serialize() , function(data){
							//alert(data);
							if (data == 2){
								$( "#hidden-message-input" ).dialog({
									resizable: false,
									width: 400,
									modal: true,
									buttons: {
								...
								$.cookie('cookie_spam', 'spam_<? echo $su_short_url ?>', { expires: 60*60*1000 });
								$( "#hidden-message-spam-thanks" ).dialog({
									resizable: false,
									width: 400,
									modal: true,
									buttons: {
	...
	$( "a#scrap_input" ).click(function() {
		$.post('/process/scrap.php', {mode:"write", user:"<? echo $_GET['user'] ?>", num:<? echo $_GET['id'] ?>, short_url:"<? echo $su_short_url ?>"}, function(data){
			//$("#temp").text(data);
			if (data == 0){
				$( "#hidden-message-scrap-err" ).dialog({
					resizable: false,
					width: 400,
					modal: true,
					buttons: {
	...
	$( "a#good" ).click(function() {
		if ($.cookie('cookie') != 'nogood_<? echo $_GET['id'] ?>'){
			$.post('/process/nogood.php', {mode:"good", user:"<? echo $_GET['user'] ?>", num:<? echo $_GET['id'] ?>},
				function(data){
	...
	$( "a#nogood" ).click(function() {
		if ($.cookie('cookie') != 'nogood_<? echo $_GET['id'] ?>'){
			$.post('/process/nogood.php', {mode:"nogood", user:"<? echo $_GET['user'] ?>", num:<? echo $_GET['id'] ?>},
				function(data){
					$.cookie('cookie', 'nogood_<? echo $_GET['id'] ?>', { expires: 60*60*1000 });
...
<div id="hidden-message-spam" title="신고하기" style="display:none">
    <form id="spam">
    <input type="hidden" value="<? echo $su_short_url ?>" name="url" />
    <input type="hidden" value="<? echo $_GET['user'] ?>" name="url_user" />
    
    <table width="100%" border="0" cellspacing="0" cellpadding="1">
    	<tr>
        	<td colspan="2" style="padding-top:15px">여러분의 적극적인 관심과 신고가 건전한 인터넷 문화를 만듭니다.<br />허위신고일 경우, 신고자의 서비스 활동이 제한될 수 있으니, 유의하시어 신중하게 신고해 주세요. <br />명예훼손, 저작권 등 신고자의 권리가 침해된 경우에는 권리침해 신고센터로 문의해주세요.</td>
        </td>    
        <tr>
            <td width="50%" style="padding-top:15px"><input type="radio" id="spam_radio" name="reason" value="0" />&nbsp;영리목적/홍보성</td>
            <td style="padding-top:15px"><input type="radio" id="spam_radio" name="reason" value="1" />&nbsp;음란성/선정성</td>
        </tr>
        <tr>
            <td width="50%" ><input type="radio" id="spam_radio" name="reason" value="2" />&nbsp;불법정보</td>
            <td><input type="radio" id="spam_radio" name="reason" value="3" />&nbsp;욕설/인신공격</td>
        </tr>
        <tr>
            <td width="50%"><input type="radio" id="spam_radio" name="reason" value="4" />&nbsp;개인정보노출</td>
            <td><input type="radio" id="spam_radio" name="reason" value="5" />&nbsp;내용의 반복 게시 (도배)</td>
        </tr>
        <tr>
            <td width="50%"><input type="radio" id="spam_radio" name="reason" value="9" />&nbsp;기타</td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            <td colspan="2" style="padding-top:15px">기타 항목 선택 후 입력해주세요. (최대 200자)</td>
        </tr>
        <tr>
            <td colspan="2"><textarea style="height:50px; width:98%;" maxlength="200" class="ui-widget-content ui-corner-all" name="andsoon" /></textarea></td>
        </tr>
    </table>
    </form>
</div>

<div id="hidden-message-input" style="display:none">
	<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 25px 0;"></span>입력해주세요.</p>
</div>
<div id="hidden-message-andsoon" style="display:none">
	<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 25px 0;"></span>기타 사유를 입력해주세요.</p>
</div>
<div id="hidden-message-spam-thanks" style="display:none">
	<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>신고했습니다. <br />여러분의 적극적인 관심과 신고가 건전한 인터넷 문화를 만듭니다.<br />고맙습니다.</p>
</div>
<div id="hidden-message-scrap" style="display:none">
	<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>이 글을 스크랩 하였습니다.<br />지금 스크랩을 확인하시겠습니까?</p>
</div>
<div id="hidden-message-scrap-err" style="display:none">
	<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>이미 스크랩하신 글 입니다.</p>
</div>
...
<?
//임시
//session_unregister("set_facebook_short");
session_unregister("set_subject_short");
?>
